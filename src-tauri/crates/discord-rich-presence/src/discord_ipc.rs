...
pub trait DiscordIpc {
    ...
    //         if self.get_connected() { continue };

    //         if self.connect().is_ok() { println!("Connected to Discord IPC client!") }
    ...
    fn set_activity_safe(&mut self, activity_payload: Activity) -> Result<()> {

        if !self.get_connected() { return Ok(()) }

        if self.set_activity(activity_payload.clone()).is_err() { self.reconnect()? }

        self.set_activity(activity_payload)?;

        Ok(())

    }

    fn clear_activity_safe(&mut self) -> Result<()> {

        if !self.get_connected() { return Ok(()) }

        if self.clear_activity().is_err() { self.reconnect()? }
